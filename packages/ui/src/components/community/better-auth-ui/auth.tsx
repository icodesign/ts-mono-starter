...
export function Auth({ className, path, socialLayout, socialPosition, view }: AuthProps) {
  ...
  for (const plugin of plugins) {
    const authPlugin = plugin as AuthPluginViews;
    const pluginAuthPaths = authPlugin.viewPaths?.auth;
    ...
    const PluginView = authPlugin.views?.auth?.[pluginView];
    if (!PluginView) continue;
  ...
  if (authView === "signIn" && !emailAndPassword?.enabled) {
    const Fallback = (plugins as AuthPluginViews[]).find(
